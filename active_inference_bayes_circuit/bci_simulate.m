function sim = bci_simulate(net, input, program, cfg)
%BCI_SIMULATE Deterministic rollout through the repository's runCTRNN.
% sim = bci_simulate(net, input, program, cfg)
...
assert(size(input, 2) == 2, 'bci:PhysicalInputDimension', 'Physical input must have two columns.');
...
assert(all(input(:) >= 0 & input(:) <= 1), ...
    'bci:InputRange', 'Multiplier inputs must remain in [0,1].');
if size(input, 1) == 1
...
if ~isempty(program)
    ...
    assert(all(program >= 0 & program <= 1), ...
        'bci:ProgramRange', 'Encoded weights must remain in [0,1].');
    externalInput = [input, repmat(program, size(input, 1), 1)];
else
    externalInput = input;
end
assert(size(externalInput, 2) == net.numExternalInput, ...
    'bci:InputDimension', 'Physical input plus code has the wrong dimension.');
assert(cfg.dt <= min(net.tau), 'bci:TimeStep', ...
