...
% 
    ...
    for n=1:nTimes
        ...
        while c<=AM_CYCLES && isFix(n)==0
            for steps=1:AM_STEP
                k=k+1;
                out(k,:)=beta .*out(k-1,:)+ alfa .*(feval(net.outputFun,out(k-1,:)*iW+...
                        externalInput(n,:)*eW+net.biases));
            end
        ...
        for steps=1:AM_REMAINDER
            k=k+1;
            out(k,:)=beta .*out(k-1,:)+ alfa .*(feval(net.outputFun,out(k-1,:)*net.internalWMatrix+...
                     externalInput(n,:)*net.externalWMatrix+net.biases));
        end
