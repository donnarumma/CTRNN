...
assert(exist(fullfile(paths.ctrnn_root, 'mulation.m'), 'file') == 2, ...
    'ai2:CTRNNMissing', 'CTRNN root does not contain mulation.m: %s', paths.ctrnn_root);
assert(exist(fullfile(paths.spm_root, 'toolbox', 'DEM', 'spm_MDP_VB_X.m'), 'file') == 2, ...
    'ai2:SPMMissing', 'SPM12 MDP solver not found at %s', paths.spm_root);
...
for k = 1:numel(files)
    ...
    assert(fid >= 0,'ai2:SourceRead','Cannot read source %s',files{k});
    guard = onCleanup(@() fclose(fid));
    bytes = fread(fid,Inf,'*uint8');
    clear guard
    fid = fopen(fullfile(destination,files{k}),'wb');
    assert(fid >= 0,'ai2:CacheWrite','Cannot cache source %s',files{k});
    guard = onCleanup(@() fclose(fid));
    count = fwrite(fid,bytes,'uint8');
    assert(count == numel(bytes),'ai2:CacheWrite','Incomplete cached file.');
    clear guard
end
end
